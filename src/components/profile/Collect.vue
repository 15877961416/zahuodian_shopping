  ...
  <div style="position: relative;">
    ...
    <div class="item_shopping">
      <el-checkbox-group v-model="checkList" style="font-size: 0;">
        <ul v-for="(item,index) in this.$store.state.Profile.Collect">
          <li>
  ...
  export default {
    ...
    methods: {
      check_all() {
        ...
        if (this.checked) {
          for (let i = 0; i < this.$store.state.Profile.Collect.length; i++) {
            this.checkList.push(i)
          }
        }
      },
      get_check(index) {
        this.checked = this.checkList.length !== this.$store.state.Profile.Collect.length ? false : true
      },
      delete_check() {
        while (this.checkList.length) {
          let once = this.checkList.pop()
          var arr = JSON.parse(JSON.stringify(this.$store.state.Profile.Collect))
          // console.log(this.$route.name, this.goods[once].id);
          // this.$api.deleteGoods(this.$route.name, this.goods[once].id, {})
          arr.splice(once, 1)
          // console.log(arr); 
          this.$store.commit('Delete_Collect', arr)
          // console.log(this.check_list[i]);
          // console.log(this.check_list.length);
        }
        this.$api.patchCollect(sessionStorage.getItem('token'), { Collect: arr })
